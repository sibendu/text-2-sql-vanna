...
async def on_chat_start():
    ...
    azure_openai_deployment = os.getenv("AZURE_OPENAI_GPT4O_DEPLOYMENT_NAME")
    azure_api_version = os.getenv("AZURE_API_VERSION", "2024-12-01-preview")
    azure_api_key = os.getenv("AZURE_OPENAI_API_KEY")
    
    # Validate required environment variables
    if not all([azure_openai_endpoint, azure_openai_deployment, azure_api_key]):
        raise ValueError(
            "Missing required Azure OpenAI environment variables: "
            "AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_GPT4O_DEPLOYMENT_NAME, AZURE_OPENAI_API_KEY"
        )
